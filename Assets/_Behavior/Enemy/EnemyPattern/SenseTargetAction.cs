...
[NodeDescription(name: "Sense Target", story: "[Self] senses [Target] and updates", category: "Action", id: "724a39702398674958d43ac19cea462c")]
public partial class SenseTargetAction : Action
{
    ...
    protected override Status OnUpdate()
    {
        EnemyAIDebugVisualizer debugVisualizer = Self?.Value != null
            ...
            : null;        EnemyAIConsoleMonitor consoleMonitor = Self?.Value != null
        ...
        EnemyData config = bridge != null ? bridge.Config : null;

        // 삼항 연산자(?:)를 사용하여 Config가 없을 경우 기본값을 할당합니다. (에러 방지)
        float attackRange = bridge != null ? bridge.GetMinAttackRange() : 2f;
        ...
        if (visible)
        {
            //Debug.Log($"<color=yellow>[AI Flow 1] 플레이어 포착 완료!</color> " +
            //  $"| 블랙보드 거리(DistanceToTarget): {DistanceToTarget.Value:F2}m " +
            //  $"| 블랙보드 사거리(AttackRange): {bridge.GetMinAttackRange():F2}m " +
            //  $"| 시야확보(CanSeeTarget): {CanSeeTarget.Value}");
        ...
        else if (HasLastKnownPosition.Value)
        {
            ...
            float investigateArrivalTolerance = Mathf.Max(
                0.3f,
                bridge != null ? attackRange * 0.5f : DefaultInvestigateArrivalTolerance);
        ...
        if (debugVisualizer != null)
        {
            ...
            if (visible && planarDistance <= attackRange)
            {
