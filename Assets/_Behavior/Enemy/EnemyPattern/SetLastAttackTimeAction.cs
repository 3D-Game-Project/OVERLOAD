...
public partial class SetLastAttackTimeAction : Action
{
    ...
    protected override Status OnUpdate()
    {
        ...
        if (!isWaitingForAttackWindow)
        {
            ...
            //Debug.Log($"<color=magenta>[AI Flow 4] 공격 조준(Windup) 시작.</color> 선딜레이 타이머 작동: {waitSeconds:F2}초 대기...");
        ...
        if (bridge != null && targetPlayer != null)
        {
            Vector3 targetAimPoint = targetPlayer.transform.position + Vector3.up * 1f;

            //Debug.Log("<color=red>[AI Flow 5] 조준 완료!</color> 비헤이비어 트리에서 무기 브릿지로 전탄 격발 명령 하달.");

            //Debug.Log($"SetLasAttackTimeAction, {targetAimPoint}");
