...
public class EnemyAIDebugVisualizer : MonoBehaviour
{
    ...
    private void DrawDebugGizmos()
    {
        ...
        SenseSnapshot snapshot = BuildSenseSnapshot(currentBridge, selectedTargetPosition);
    ...
    private SenseSnapshot BuildSenseSnapshot(EnemyBehaviorBridge currentBridge, Vector3 targetPosition)
    {
        EnemyData config = currentBridge.Config; // 브릿지에서 config를 꺼내옴
        ...
        SenseSnapshot snapshot = new SenseSnapshot
        {
            ...
            // 🌟 [핵심 수정] 에디터 모드에서도 안전한 currentBridge를 사용하여 사거리를 가져옵니다!
            attackRange = currentBridge.GetMinAttackRange(),
    ...
    private void DrawSensorGizmos(SenseSnapshot snapshot, EnemyData config)
    {
